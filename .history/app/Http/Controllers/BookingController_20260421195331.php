...
class BookingController extends Controller
{
    ...
    public function booking(Request $request)
    {
        ...
        $request->validate([
            ...
            'jam'         => 'required',
            'durasi'      => 'required', // Pastikan durasi divalidasi
            'bukti'       => 'required|image|mimes:jpeg,png,jpg|max:2048',
        ]);
    
        $hargaLapangan = $request->lapangan_id;
        $hargaRaket = $request->raket_id ?? 0;
        $durasi = $request->durasi;
    
        // Hitung Total Harga (agar tidak error field default value)
        $totalHarga = ($hargaLapangan + $hargaRaket) * $durasi;
    
        // --- PENJAGA STATUS TEKNIS & BENTROK (Tetap sama seperti sebelumnya) ---
        $lapangan = \App\Models\LapanganModel::where('harga', $hargaLapangan)->first();
        if ($lapangan && trim(strtolower($lapangan->status)) === 'teknis') {
        ...
        try {
            ...
            // 3. Simpan ke Database (PASTIKAN SEMUA FIELD DIISI)
            \App\Models\BookingModel::create([
                ...
                'jam'          => $request->jam,
                'durasi'       => $durasi,         // <--- INI BIANG KEROKNYA TADI (SUDAH DITAMBAHKAN)
                'total_harga'  => $totalHarga,    // <--- INI JUGA WAJIB DIISI
                'bukti'        => $path,
                'status'       => 'pending',
            ]);
