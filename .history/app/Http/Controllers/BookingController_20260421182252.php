    ...
    public function booking(Request $request)
{
    // 1. Validasi Input
    $request->validate([
        'lapangan_id' => 'required', // Isinya HARGA (35000/50000/70000)
    ...
    // ==============================================================
    // PENJAGA 1: CEK STATUS REAL-TIME (SINKRONISASI ID & HARGA)
    // ==============================================================
    
    // Kita cari data lapangan yang harganya cocok dengan pilihan user
    $lapangan = \App\Models\LapanganModel::where('harga', $hargaInput)->first();

    // Jika lapangan ditemukan, cek statusnya
    if ($lapangan) {
        if (trim(strtolower($lapangan->status)) !== 'tersedia') {
            return back()->with('error', 'MAAF: Lapangan ' . $lapangan->nama . ' sedang FULL/TUTUP oleh Admin saat ini.')->withInput();
        }
    }

    // Cek status raket (berlaku hal yang sama)
    if ($request->raket_id != 0) {
        $raket = \App\Models\RaketModel::where('harga', $request->raket_id)->first();
        if ($raket && trim(strtolower($raket->status)) !== 'tersedia') {
            return back()->with('error', 'MAAF: Raket yang Anda pilih sedang tidak tersedia.')->withInput();
        }
    }

    // ==============================================================
    // PENJAGA 2: CEK JADWAL BENTROK (JAM & TANGGAL)
    // ==============================================================
    ...
    if ($isFull) {
        return back()->with('error', 'MAAF: Jadwal ini sudah dipesan orang lain. Silakan pilih jam lain.')->withInput();
    }
    ...
    try {
        ...
        return redirect('/history')->with('success', 'Booking berhasil! Menunggu konfirmasi admin.');
