                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="select-raket" name="raket_id" class="w-full p-4 bg-[#0a0a0a] text-white border border-white/10 rounded-2xl outline-none focus:border-neon focus:ring-1 focus:ring-neon transition-all appearance-none cursor-pointer">
                                    <option value="" data-price="0" data-name="Tanpa Raket" {{ old('raket_id') == '' ? 'selected' : '' }}>Tidak Sewa</option>
                                    
                                    @foreach($raket as $item)
                                        @if($item->status == 'tersedia' && $item->stok > 0)
                                            <option value="{{ $item->id }}" data-price="{{ $item->harga }}" data-name="{{ $item->nama }}" {{ old('raket_id') == $item->id ? 'selected' : '' }}>
                                                {{ $item->nama }} - Rp {{ number_format($item->harga, 0, ',', '.') }}
                                            </option>
                                        @else
                                            <option value="{{ $item->id }}" data-price="{{ $item->harga }}" data-name="{{ $item->nama }}" disabled>
                                                {{ $item->nama }} - Rp {{ number_format($item->harga, 0, ',', '.') }} (Stok Habis / Kendala)
                                            </option>
                                        @endif
                                    @endforeach
                                </select>
...
<script>
    ...
    function updateSummary() {
        let hargaLapangan = parseInt(selLapangan.value) || 0;
        
        // Mengambil harga dari atribut data-price yang baru ditambahkan
        let hargaRaket = parseInt(selRaket.options[selRaket.selectedIndex].getAttribute('data-price')) || 0;
